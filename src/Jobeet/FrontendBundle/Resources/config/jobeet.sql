...
CREATE TABLE IF NOT EXISTS `categoryaffiliate` (
  ...
  PRIMARY KEY (`id`),
  KEY `categoryaffiliate_FI_2` (`affiliate_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=5;
...
--
-- Megkötések a táblához `categoryaffiliate`
--
ALTER TABLE `categoryaffiliate`
  ADD CONSTRAINT `categoryaffiliate_FK_1` FOREIGN KEY (`category_id`) REFERENCES `category` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `categoryaffiliate_FK_2` FOREIGN KEY (`affiliate_id`) REFERENCES `affiliate` (`id`) ON DELETE CASCADE;

--
-- Megkötések a táblához `job`
--
ALTER TABLE `job`
  ADD CONSTRAINT `job_FK_1` FOREIGN KEY (`category_id`) REFERENCES `category` (`id`);